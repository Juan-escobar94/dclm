...
class BertForQualityRegression(BertForSequenceClassification):
    def __init__(self, config):
        ...
        # Linear Regression head: With this we map the [CLS] pooled output to one value
        self.regressor = nn.Linear(config.hidden_size, 1)
        # Squash the value into (0,1) using Sigmoid activation 
        self.sigmoid = nn.Sigmoid()

        # init weights (from parent function) 
        self.init_weights()
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None, **kwargs):
        ...
        # pooler output contains the [CLS] token representation
        pooled_output = outputs.pooler_output  
        # apply dropout before passing to the Linear Layer
        pooled_output = self.dropout(pooled_output) 
        logits = self.regressor(pooled_output)
        # We apply sigmoid fn to get an output in the desired range for quality scoring: (0, 1) 
        probs = self.sigmoid(logits)
        loss = None
        if labels is not None:
            # We use MSELoss since we are dealing with a regression task
            loss_fct = nn.MSELoss()
